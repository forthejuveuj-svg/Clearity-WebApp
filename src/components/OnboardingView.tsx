        ...
        <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl md:rounded-3xl lg:rounded-3xl p-4 md:p-6 lg:p-8">
          ...
          {recentSessions.map((session) => (
            <div
              ...
              className="bg-white/5 backdrop-blur-sm border border-white/20 rounded-xl md:rounded-2xl lg:rounded-2xl p-4 md:p-5 lg:p-6 
                         hover:bg-white/10 hover:border-blue-500/40 hover:shadow-lg hover:shadow-blue-500/10
            ...
            >
              <div className="flex items-start justify-between mb-3 md:mb-3 lg:mb-4">
                <div className="flex items-center gap-2 md:gap-2 lg:gap-3">
                  <div className="w-8 h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 rounded-lg bg-gradient-to-br from-purple-500/50 to-blue-500/50 
                                  flex items-center justify-center group-hover:scale-110 transition-transform">
                    <Brain className="w-4 h-4 md:w-4 md:h-4 lg:w-5 lg:h-5 text-white" />
                  </div>
                  <div>
                    <h3 className="text-base md:text-base lg:text-lg font-medium text-white group-hover:text-blue-300 transition-colors">{session.title}</h3>
                    <div className="flex items-center gap-1 md:gap-1 lg:gap-2 text-xs md:text-xs lg:text-sm text-gray-400">
                      <Calendar className="w-3 h-3" />
                      <span>{session.date}</span>
                    </div>
                  </div>
                </div>
                <div className="px-2 md:px-2 lg:px-3 py-0.5 md:py-0.5 lg:py-1 bg-blue-500/20 rounded-full text-xs md:text-xs lg:text-sm text-blue-300">
                  {session.nodes} thoughts
                </div>
              </div>
              
              <p className="text-xs md:text-xs lg:text-sm text-gray-300 line-clamp-2 group-hover:text-white transition-colors">
                {session.preview}
              </p>
            </div>
          ))}
